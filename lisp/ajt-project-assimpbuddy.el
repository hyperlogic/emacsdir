...
(setq compile-command (concat "cd " ajt-assimpbuddy-path " && ./lint.sh && cd hypercore && ./lint.sh && cd ../build && NO_COLOR=true cmake --build . --config Debug"))
...
(defun ajt-assimpbuddy-cpp-search (arg)
  ...
  (ajt-ripgrep-find arg (concat ajt-assimpbuddy-path) '("*.h" "*.c" "*.cc" "*.cpp")))
